...
function add_item_to_table(e, t, a, i, iid=0) {
  ...
  if ("" !== (e = void 0 === e || "undefined" == e ? get_item_preview_values() : e).description || "" !== e.long_description || "" !== e.rate) {
    ...
    return get_taxes_dropdown_template(l, e.taxname).done((function(a) {
      ...
      var images = $('input[name="new_item_images"]').val();
      var imagesArray = [];
      if (images !== undefined && images !== null) {
          var imagesArray = images.split(',');
      }

      // Loop through the imagesArray and create HTML elements for each image
      var baseUrl = $('#base-url').data('base-url');
      var imageHTML = '';
      imagesArray.forEach(function(image) {
        var imageName = image.replace(/\s+/g, '_');
          imageHTML += '<img onclick="previewImage(this)" class="preview-image" src="' + baseUrl + 'assets/invoice_items/thumb/' +imageName + '" alt="Image">';
      });
      $('.show_tr').css('display','none');
      // alert(imageHTML);
      $('.set_item_images').html('&nbsp');
      $('.get_item_images').html('');

      if(iid == 0){
        manage_invoice_item(e, function(item_id) { 
          
        $('input[name="newitems[' + s + '][item_id]"]').val(item_id);

      
      // reset onclick
      $('.btnSetForItemId').attr('onclick', "add_item_to_table('undefined','undefined',1); return false;");
      
      if(iid != 0){
        $('input[name="newitems[' + s + '][item_id]"]').val(itemid);
      }

      n += '<input type="hidden" class="order" name="newitems[' + s + '][order]">';
      n += "</td>";
      n += '<td class="bold description"><input type="hidden" id="set_item_id" name="newitems[' + s + '][item_id]" value="' + itemid + '">';
      n += '<textarea name="newitems[' + s + '][description]" class="form-control" rows="5">' + e.description + "</textarea></td>";
      ...
      })), n += '<td><input type="number" min="0" onblur="calculate_total();" onchange="calculate_total();" data-quantity name="newitems[' + s + '][qty]" value="' + e.qty + '" class="form-control">', e.unit && void 0 !== e.unit || (e.unit = ""), n += '<input type="text" placeholder="' + app.lang.unit + '" name="newitems[' + s + '][unit]" class="form-control input-transparent text-right" value="' + e.unit + '">', n += "</td>", n += '<td class="rate"><input type="number" data-toggle="tooltip" title="' + app.lang.item_field_not_formatted + '" onblur="calculate_total();" onchange="calculate_total();" name="newitems[' + s + '][rate]" value="' + e.rate + '" class="form-control"></td>', n += '<td class="taxrate">' + a + "</td>", n += '<td><input type="hidden" name="get_rand_number" class="set_rand_number_invoice" value="0"><label for="item_images_invoice" class="file-upload"><input type="file" multiple id="item_images_invoice" onchange="item_img_invoice(' + item_id + ', '+ s +')"  class="form-control file-input"><i class="fas fa-upload"></i> Upload</label></td>', n += '<td class="amount" align="right">' + format_money(o, !0) + "</td>", n += '<td><a href="#" class="btn btn-danger pull-left" onclick="delete_item(this,' + t + ','+ s +'); return false;"><i class="fa fa-trash"></i></a></td>', n += "</tr>", n += "<tr class='remove_images"+s+"'><td colspan='4'>&nbsp;</td><td style='padding-left: 0px;' colspan='6' align='right'><div style='white-space: nowrap;'>"+imageHTML+"</div></td></tr>"; $("table.items tbody").append(n), $(document).trigger({
...
//     // table_row += '<td><input type="hidden" name="get_rand_number" class="set_rand_number_invoice" value="0">' +
//     //   '<label for="item_images_invoice" class="file-upload">' +
//     //   '<input type="file" multiple id="item_images_invoice" onchange="item_img_invoice(\'' + itemid + '\',5)" class="form-control file-input">' +
