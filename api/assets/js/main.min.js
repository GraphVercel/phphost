...
function add_item_to_preview(e) {
	...
	requestGetJSON("invoice_items/get_item_by_id/" + e).done((function(e) {
		console.log(e);
		// added by ak
		$('.btnSetForItemId').attr('onclick', "add_item_to_table('undefined','undefined',1,'undefined','" + e.itemid + "'); return false;");
		getImagesbyitemid(e.itemid).done(function(images) {
          	
            $('.set_item_images').html('<input type="hidden" name="new_item_images" value="'+images+'">');
			var imagesContainer = $('.get_item_images');
			imagesContainer.html(''); 
			$('.show_tr').css('display','table-row');

			// Iterate over the images array and create HTML elements for each image
			var baseUrl = $('#base-url').data('base-url');
			var imagesCount = images.length;
			var imagesPerRow = 2; // Change this value if you want a different number of images per row
			var imagesInCurrentRow = 0;

			images.forEach(function(image, index) {
			    var imgElement = $('<img onclick="previewImage(this)" class="preview-image">').addClass('item-image').attr('src', baseUrl + 'assets/invoice_items/thumb/' + image);
			    imagesContainer.append(imgElement);
			});

        }).fail(function(jqXHR, textStatus, errorThrown) {
            console.error("Error fetching images:", errorThrown);
        });
		// end by ak end
		clear_item_preview_values(), $('.get_itemid').html('<input type="hidden" name="itemid" value="'+e.itemid+'">'),$('.main textarea[name="description"]').val(e.description), $('.main textarea[name="long_description"]').val(e.long_description.replace(/(<|&lt;)br\s*\/*(>|&gt;)/g, " ")), _set_item_preview_custom_fields_array(e.custom_fields), $('.main input[name="quantity"]').val(1);
		var t = [];
...
function getImagesbyitemid(itemID) {
    return requestGetJSON("invoice_items/get_item_images/" + itemID);
}
...
function clear_item_preview_values(e) {
	var t = $("table.items tbody").find("tr:last-child").find("select").selectpicker("val"),
		a = $(".main");
	a.find("textarea").val(""), a.find('td.custom_field input[type="checkbox"]').prop("checked", !1), a.find("td.custom_field input:not(:checkbox):not(:hidden)").val(""), a.find("td.custom_field select").selectpicker("val", ""), a.find('input[name="quantity"]').val(1), a.find("select.tax").selectpicker("val", t), a.find('input[name="rate"]').val(""), a.find('input[name="unit"]').val(""), $('input[name="task_id"]').val(""), $('input[name="expense_id"]').val(""), $('input[name="itemid"]').val("")
}
...
function delete_item(e, t, s='') {
	// console.log(e);
	$(e).parents("tr").addClass("animated fadeOut", (function() {
		setTimeout((function() {
			$(".remove_images"+s).remove();
