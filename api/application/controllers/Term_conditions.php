...
class Term_conditions extends ClientsController
{
    ...
    public function add_signature($id,$hash){
        process_digital_signature_image($this->input->post('signature', false), LEAD_ATTACHMENTS_FOLDER.'/' . $id);

        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'leads', get_terms_signature_acceptance_info_array());
    ...
    public function add_card_information($id,$hash){
        $CI        = &get_instance();
        if(!empty($CI->input->post())){
            $card_number=$CI->input->post('card_number');
            $card_name=$CI->input->post('card_name');
            $card_date=$CI->input->post('card_date');
            $card_vv=$CI->input->post('card_vv');
