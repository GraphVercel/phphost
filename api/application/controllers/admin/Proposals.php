...
class Proposals extends AdminController
{
    ...
    public function proposal($id = '')
    {
        if ($this->input->post()) {
            ...
            $prefix = 'item_images_invoice_';
            foreach ($proposal_data as $key => $value) {
                if (strpos($key, $prefix) === 0) {
                    unset($proposal_data[$key]);
                }
            }
            //added by ak
            if(isset($proposal_data['itemid'])){
                unset($proposal_data['itemid']);
            }
            if(isset($proposal_data['get_rand_number'])){
                unset($proposal_data['get_rand_number']);
            }
            if(isset($proposal_data['item_images_invoice'])){
                unset($proposal_data['item_images_invoice']);
            }
            //
            if ($id == '') {
