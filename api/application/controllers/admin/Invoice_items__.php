...
class Invoice_items extends AdminController
{
    ...
    public function delete_item_images(){

        if ($this->input->post()) {

                $data = $this->input->post();
                if($data['random_number'] == 0){
                    $get_random_code = get_item_random_number_against_id($data['imgid']);
                    $data['random_number'] = $get_random_code;
                }
                
                $success = $this->invoice_items_model->item_image_delete($data);
                $images_random = item_images_load_against_random_number($data['random_number']);
                $img_id = item_img_id_against_random_number($data['random_number']);
     ...
     public function upload_item_image() {
        ...
        $img_id = item_img_id_against_random_number($random_number);
        // Return JSON response with file paths
        echo json_encode(['random_number' => $random_number, 'images_random' => $images_random, 'img_id'=> $img_id]);
    }
    ...
    public function get_item_images($itemid){
         ...
         if ($this->input->is_ajax_request()) {
            ...
            echo json_encode(['images'=>$images,'img_id'=>$img_id]);
        }
    ...
    public function get_item_by_id($id)
    {
        if ($this->input->is_ajax_request()) {
            $item                     = $this->invoice_items_model->get($id);
            $item->random_number      = item_images_load($id)[0]->key_code;
            if (!empty($item->random_number)) { 
                $item->random_number = $item->random_number;
            } else {
                $item->random_number = rand(10000, 99999);
            }
            $item->item_images      = item_images_load_against_random_number($item->random_number);
            $item->img_id           = item_img_id_against_random_number($item->random_number);
