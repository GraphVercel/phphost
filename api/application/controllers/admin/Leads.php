...
class Leads extends AdminController
{
    ...
    public function sent_email_term_conditions()
    {
        if ($this->input->post()) {
            ...
            $url=site_url('/term_conditions/index/'.$leadid.'/'.$hash);
            $template           = new StdClass();
            $template->message  = get_option('email_header') . ''.$term_description.'</br></br>&nbsp;&nbsp;'.'<a style="font-size:18px" href="'.$url.'"></br></br>&nbsp;Click on Terms</a>'. get_option('email_footer');
