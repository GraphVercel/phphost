...
class Invoices extends AdminController
{
    ...
    public function invoice($id = '')
    {
        if ($this->input->post()) {
            ...
            //added by ak
            if(isset($invoice_data['itemid'])){
                unset($invoice_data['itemid']);
            }
            if(isset($invoice_data['get_rand_number'])){
                unset($invoice_data['get_rand_number']);
            }
            if(isset($invoice_data['item_images_invoice'])){
                unset($invoice_data['item_images_invoice']);
            }
    ...
    public function get_invoice_data_ajax($id)
    {
        ...
        $data['documnets'] = $this->invoices_model->get_invoice_document($id);
        $token = bin2hex(random_bytes(16));
        $baseUrl = "https://app.graphiteartistries.com/Fileupload";
        $secureUrl = $baseUrl . "?token=" . urlencode($token) . "&id=" . urlencode($id);
        $data['qr'] = $this->generate_qrcode($secureUrl);
    ...
    public function delete_invoice_attachments(){
       if($this->input->post(id)){
           $response = $this->invoices_model->delete_invoice_attachments($this->input->post(id));
           if($response){
