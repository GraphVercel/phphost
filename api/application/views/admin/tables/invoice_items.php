...
$result  = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, ["WHERE added_by = 'item'"], $additionalSelect);
...
foreach ($rResult as $aRow) {
    ...
    // show images on table
        $item_images = item_images_load($aRow['id']);
        $images = [];
        $imageHtml = ''; // To hold the HTML for images

        // Counter to keep track of images
        $count = 0;

        foreach ($item_images as $img) {
            // Add the image HTML to the array
            $imageHtml .= '<div style="display: inline-block;">'; // Set inline display and margin
            $imageHtml .= '<img class="preview-image" src="' . base_url() . 'assets/invoice_items/thumb/' . $img->image . '" style="width:50px;" onclick="previewImage_item(this)">';
            $imageHtml .= '</div>';

            // Increment the counter
            $count++;

            // Check if the counter reaches 2 (for every two images)
            if ($count % 2 == 0) {
        ...
        // If the total number of images is odd, add an extra line break
        if ($count % 2 != 0) {
            $imageHtml .= '<br>';
        }

        // Add the image HTML to the row
        $row[] = $imageHtml;
