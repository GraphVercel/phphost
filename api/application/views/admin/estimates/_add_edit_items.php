...
<div class="panel-body">
    ...
    <div class="col-md-8 col-md-offset-4">
        <table class="table text-right">
            <tbody>
                <?php 
                $send_total_checked = false;
                if (isset($estimate) && is_object($estimate) && property_exists($estimate, "send_total")) {
                    // For existing estimates
                    $send_total_checked = $estimate->send_total == 1;
                } elseif (!isset($estimate)) {
                    // For new estimates
                    $send_total_checked = true;
                }
                ?>
                <tr id="manage_visibility">
                    <td><span class="bold tw-text-neutral-700">Send Total:</span></td>
                    <td>
                        <input type="checkbox" id="commentCheckbox" class="checkbox pull-right" onchange="sendtotalamount(this.checked)"
                            <?php echo $send_total_checked ? 'checked' : ''; ?>>
                    </td>
                </tr>
                <tr id="subtotal">
...
<script type="text/javascript">
        ...
        $.ajax({
            ...
            success: function(response){
                if(response != 0){
                    ...
                    $('.set_rand_number_invoice').val(response);
                    ...
                    $('.set_item_images').html('<input type="hidden" name="new_item_images" value="' + imageNames.join(',') + '">');
 ...
 // added by ak
   document.getElementById('item_images_invoice').addEventListener('change', function() {
    ...
    function sendtotalamount(checked) {
        var totalRow = document.getElementById("manage_visibility");
        if (checked) {
             var nextSibling = totalRow.nextElementSibling;
             while (nextSibling) {
                nextSibling.style.display  = 'table-row';
                nextSibling = nextSibling.nextElementSibling;
             }
             document.getElementById("send_total").value = 1;
        } else {
            var nextSibling = totalRow.nextElementSibling;
             while (nextSibling) {
            ...
            document.getElementById("send_total").value = 0;
        }
    }
    setTimeout(() => {
    sendtotalamount(<?php echo json_encode($send_total_checked); ?>);
    },200);
</script>