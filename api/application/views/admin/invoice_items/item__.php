...
<div class="modal fade" id="sales_item_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    ...
                    <div id="get-base-url" data-get-base-url="<?= base_url(); ?>"></div>
                </div>
                ...
                <div id="image-modal">
                    <div id="modal-content">
                        <span class="close" onclick="closeModal_it()">&times;</span>
                        <img id="modal-image">
                    </div>
                </div>
    ...
    <div class="modal-footer">
        ...
        <button type="submit" class="btn btn-primary" onclick="refreshtable()"><?php echo _l('submit'); ?></button>
        <?php echo form_close(); ?>
    </div>
</div>
</div>
...
<script>
  ...
  var modal = document.getElementById("image-modal");
   function previewImageItem(image) {
...
function item_img() {
        ...
        var fd = new FormData();
        var random_number = $('.set_rand_number').val();
        // alert(random_number)
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                if(responseData.random_number != 0){
                    ...
                    $('.set_rand_number').val(responseData.random_number);
                    $('.show_tr').css('display','table-row');
                    // fetch images loop
                    images = responseData.images_random;
                    img_id = responseData.img_id;
                    // 
                    // alert(responseData.random_number);
                    $('#uploaded_images').empty();
                    images.forEach(function(image, index) {

                    var imgContainer = $('<div>').addClass('item-container-item');
                    var imgElement = $('<img onclick="previewImageItem(this)" class="preview-image">').addClass('item-image').attr('src', '<?=base_url();?>assets/invoice_items/thumb/' + image);
                    var crossIcon = $('<span>').addClass('cross-icon-item').html('&times;').on('click', function() {
                        uploadedItemImgDelete(img_id[index]);
                    });
                        imgContainer.append(imgElement, crossIcon);
                        $('#uploaded_images').append(imgContainer);
                    });
                    // end loop
                    $('.set_item_images').html('<input type="hidden" name="new_item_images" value="' + images.join(',') + '">');
    ...
    $("body").on('show.bs.modal', '#sales_item_modal', function (event) {
        ...
        if (typeof (id) !== 'undefined') {
            ...
            requestGetJSON('invoice_items/get_item_by_id/' + id).done(function (response) {
                ...
                $itemModal.find('input[name="get_rand_number"]').val(response.random_number);
                ...
                // Get images by ak
                var imagesArray = [];
                var img_id = response.img_id;

                if (typeof response.item_images === 'string') {
                    imagesArray = response.item_images.split(',');
                } else {
                    imagesArray = response.item_images;
                }

                var baseUrl = $('#get-base-url').data('get-base-url');
                var imageHTML = '';

                imagesArray.forEach(function(image, index) {
                    var imageName = image.replace(/\s+/g, '_');
                    var imageUrl = baseUrl + 'assets/invoice_items/thumb/' + imageName;
                    imageHTML += '<div class="item-container-item">';
                    imageHTML += '<img onclick="previewImageItem(this)" class="preview-image" src="' + imageUrl + '" alt="Image">';
                    imageHTML += '<span class="cross-icon-item" onclick="uploadedItemImgDelete(\'' + img_id[index] + '\')">&times;</span>';
                    imageHTML += '</div>';
                });

                $('#uploaded_images').append('<div class="get_item_images" style="white-space: nowrap;">' + imageHTML + '</div>');
                // End by ak
...
function validate_item_form(){
 ...
 <script>
      // Function to close modal
        function closeModal_it() {
            var modal = document.getElementById('image-modal');
            modal.style.display = 'none';
        }
    // added by ak
        // for image delete
        function uploadedItemImgDelete(imgid) {
            ...
            var url = '<?=base_url();?>admin/invoice_items/delete_item_images';
