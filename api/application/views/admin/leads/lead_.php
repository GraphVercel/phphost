...
<div class="modal-body">
    ...
    echo ' lead-new';
                        ...
                        ?>
                            ...
                            <li role="presentation">
                                <a href="#term_conditions" aria-controls="term_conditions" role="tab" data-toggle="tab">
                                    <?php echo _l('term_conditions'); ?>
                                </a>
                            </li>
                            <li role="presentation">
                                <a href="#card_info" aria-controls="card_info" role="tab" data-toggle="tab">
                                    <?php echo _l('card_info'); ?>
                                </a>
                            </li>
            ...
            <div class="tab-content">
                ...
                <div role="tabpanel" class="tab-pane" id="term_conditions">
                    <?php if(empty($lead->acceptance_firstname)){?>
                    <div class="col-md-10">
                        <input type="hidden" id="hash" name="hash" value="<?php echo $lead->hash; ?>"/>
                        <?php $value = (isset($lead) ? $lead->email : ''); ?>
                        <?php echo render_input('email_address', 'email_address', $value); ?>
                        <?php $value = (isset($lead) ? $lead->email_subject : ''); ?>
                        <?php echo render_input('email_subject', 'email_subject', $value); ?>
                        <p class="bold"><?php echo _l('project_description1'); ?></p>
                         <?php $contents = ''; if (isset($lead)) {
                            $contents           = $lead->term_description;
                        } ?>
                        <textarea id="term_description" class="form-control" name="term_description" rows="6" cols="90"><?php echo $contents?></textarea></br>
                        <?php //echo render_textarea('term_description', '', $contents, [], [], '', 'tinymce'); ?>
                        <?php
                        // onclick and onfocus used for convert ticket to task too
                        //    echo render_textarea('term_description', '', $contents, ['rows' => 4, 'data-task-ae-editor' => true, !is_mobile() ? 'onclick' : 'onfocus' => (!isset($lead) || isset($lead) && $task->term_description == '' ? 'init_editor(\'.tinymce-term_description\', {height:200, auto_focus: true});' : '')], [], 'no-mbot', 'tinymce-term_description'); ?>
                        <div class="form-group form-check">
                            <input type="checkbox" class="form-check-input" id="term_check" name="term_check" onclick="termcheck()">
                            <label class="form-check-label" for="term_check">Include form to save card on file</label>
                        </div>
                        
                        <div class="text-right">
                            <button onclick="copy()" class="btn btn-primary"><?php echo _l('copy_message'); ?></button>
                            <button id="term_send_mail" onclick="termsendmail()" class="btn btn-primary"><?php echo _l('send_email'); ?></button>
                        </div>
                    </div>
                    <?php }else{?>
                    <div class="col-md-10">
                        <h2>Terms & Services Agreement</h2>
                        <div class="col-md-12" style="padding: 10px;border: 1px solid #aeaaaa">
                            <div class="col-sm-12">
                                <?php echo $lead->term_description; ?></br></br>
                            </div>
                            <div class="col-sm-6">First Name: 
                                <input type="text" style="background-color: #ffffff;color: #000000;" readonly name="acceptance_firstname" id="acceptance_firstname" placeholder="First name" class="form-control" required="true" value="<?php echo (isset($lead) ? $lead->acceptance_firstname : '') ?>">
                            </div>
                            <div class="col-sm-6">
                                Last Name:
                                <input type="text" style="background-color: #ffffff;color: #000000;" readonly name="acceptance_lastname" id="acceptance_lastname" placeholder="Last name" class="form-control" required="true" value="<?php echo (isset($lead) ? $lead->acceptance_lastname : '') ?>">
                            </div>
                            <div class="col-sm-6" style="margin-top:20px">
                                <img src="<?php echo base_url('uploads/leads/'.$lead->id.'/'.$lead->signature); ?>" class="img img-responsive"></br></br>
                            </div>
                        </div> 
                    </div>
                    <?php }?>
                ...
                <div role="tabpanel" class="tab-pane" id="card_info">
                    <div>
                        <div class="col-md-12">
                            <h3>Card on File Information
                        </div>
                        <?php if(!empty($lead->card_number)){?>
                        <div class="col-md-6">
                            <label class="form-control"><span style="font-size:15px">Card Number: </span><?php echo $lead->card_number;?></label>
                        </div>
                        <div class="col-md-6">
                            <label class="form-control"><span style="font-size:15px">Card Name: </span><?php echo $lead->card_name;?></label>
                        </div>
                        <div class="col-md-6" style="margin-top:20px">
                            <label class="form-control"><span style="font-size:15px">Date: </span><?php echo $lead->card_date;?></label>
                        </div>
                        <div class="col-md-6" style="margin-top:20px;margin-bottom:20px">
                            <label class="form-control"><span style="font-size:15px">CVV: </span><?php echo $lead->card_vv;?></label>
                        </div>
                        <?php }else{ ?>
                        <div class="col-md-12" style="margin-top:20px;margin-bottom:20px;text-align:center">
                        </br></br><h4>Data Not Found</h4></br></br>
                        </div>
                        <?php } ?>
                        <div class="clearfix"></div>
                    </div>
...
<script>
    function termcheck() {
    // Get the checkbox
    var checkBox = document.getElementById("term_check");
 
    if (checkBox.checked == true){
        var email_address = $("#email_address").val();
        var email_subject = $("#email_subject").val();
        var message = $('#term_description').val();
        ...
        $.post(admin_url + "leads/add_term_conditions", {
        leadid: aLeadId,
        term_description: message,
...
function copy() {
    ...
    function termsendmail() {
        ...
        $.post(admin_url + "leads/sent_email_term_conditions", {
        leadid: aLeadId,
        term_description: message,
        email_address: email_address,
        email_subject: email_subject,
        ...
        .done(function (response) {
            alert_float("info", 'Mail send successfully. Check your email now.');
            location.reload();
            // response = JSON.parse(response);
            // _lead_init_data(response, response.id);
        })
