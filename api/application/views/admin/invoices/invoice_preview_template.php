    ...
    [
                        ...
                        <li role="presentation" class="tab-separator">
                            <a href="#photos"
                                onclick="get_sales_notes(<?php echo $invoice->id; ?>,'invoices'); return false"
                                aria-controls="tab_notes" role="tab" data-toggle="tab">
                                <?php echo _l('photos'); ?> <span class="notes-total">
                                    <?php if ($totalNotes > 0) { ?>
                                    <span class="badge"><?php echo $totalNotes; ?></span>
                                    <?php } ?>
                                </span>
            ...
            <div class="tab-content">
                ...
                <div role="tabpanel" class="tab-pane" id="photos">
                    ...
                    <div class="row">
                    <div class="col-lg-4">
                    <?php echo '<img src="' . base_url($qr['file']) . '" />';?>
                    </div>
                    <div class="col-lg-8">
                        ...
                        <form action="">
                          ...
                          <input type="hidden" id="id" name="id" value="<?php echo $invoice->id;?>">
                        </form>
                    </div>
                    
                    </div>
                        <div class="row" id="image-preview-invoice">
                            <?php foreach ($documnets as $key => $value) { ?>
                                <div class="col-lg-2">
                                 <div class="img-blog"><span class="close_invoice" onclick="DeleteImage(this, '<?php echo $value['id']; ?>')">×</span>
                                 <img style="max-width: 100%;" onclick="openModal1('<?php echo base_url('assets/invoice_items/'.$value['file_name']); ?>')" src="<?php echo base_url('assets/invoice_items/'.$value['file_name']); ?>">
                                </div>
                                </div>
                            <?php } ?>
                        </div>
                        <!-- The Modal -->
                        <div id="imageModalDoc" class="modal">
                          <span onclick="closeModal()" class="close">&times;</span>
                          <img class="modal-content" id="modalImageDoc">
                          <div id="caption"></div>
                        </div>
                    <hr />
                    <div class="mtop20" id="sales_notes_area"></div>
...
function DeleteImage(elements, id) {
    ...
    var columnContainer = imgContainer.parentNode;
    if (id) {
        var confirmed = confirm("Are you sure you want to delete?");
        if (confirmed) {
            $.ajax({
                url: "<?=base_url();?>admin/invoices/delete_invoice_attachments",
                type: 'post',
                data: { id: id },
...
window.addEventListener('paste', e => {
    fileInput = e.clipboardData.files;
    fileSelected(fileInput, "clipboard");
});
                ...
                if (responseData.random_number !== undefined && responseData.random_number !== 0) {
                    ...
                    for (var i = 0; i < file.length; i++) {
                        (function (file_each) {
                            ...
                            var main_imagesContainer = document.createElement('div');
                            main_imagesContainer.className = "col-lg-2";
                            imagesContainer.className = 'img-blog';
                            imagesContainer.style.whiteSpace = 'nowrap';
                            var span = document.createElement('span');
                            span.innerHTML = 'x';
                            span.className = 'close_invoice';
                            var id_image = responseData.insert_id[i];
                            span.onclick = function () {
                                DeleteImage(this, id_image);
                            }
                            ...
                            main_imagesContainer.appendChild(imagesContainer);
                            previewContainer.appendChild(main_imagesContainer);
                        })(file[i]);
                    }
                    $('#myFile').val('');
                } else {
...
function openModal1(imageUrl) {
    var modal = document.getElementById('imageModalDoc');
    var modalImage = document.getElementById('modalImageDoc');
...
window.onclick = function(event) {
   if (event.target.id == "imageModalDoc") {
      $("#imageModalDoc").hide();
   }
}
</script>
