...
class Invoices_model extends App_Model
{
    ...
    public function get_invoice_document($id){
        $this->db->where('invoice_id', $id);
        return $this->db->get(db_prefix() . 'invoice_attachments')->result_array();
    }
    
    public function delete_invoice_attachments($id){
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'invoice_attachments');
        if ($this->db->affected_rows() > 0) {
            return true;
        }
        return false;
