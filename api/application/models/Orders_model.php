...
class Orders_model extends App_Model
{
    ...
    {
        $announcements = $this->db->where('order_id',$id)->get(db_prefix() . 'orders_additional_image')->result_array();
    ...
    public function delete($id){
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'orders_image');

        if ($this->db->affected_rows() > 0) {
            $this->db->where('order_id', $id);
            $this->db->delete(db_prefix() . 'orders_additional_image');
