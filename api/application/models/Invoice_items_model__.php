...
class Invoice_items_model extends App_Model
{
    ...
    public function add($data)
    {
        ...
        if (isset($data['get_rand_number'])) {
            unset($data['get_rand_number']);
        }
    ...
    public function item_image_update($data){

        $this->db->where('key_code',$data['get_rand_number'])->update(db_prefix() . 'item_images',['item_id'=>$data['item_id']]);
        return true;
    }

    public function item_image_delete($data){

        $this->db->from(db_prefix().'item_images');
        if($data['random_number'] != 0){
           $this->db->where('key_code', $data['random_number']);
        }
