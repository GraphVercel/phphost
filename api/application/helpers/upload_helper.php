...
function create_img_invoice_thumb($path, $filename, $width = 300, $height = 300)
{
    $CI = &get_instance();

    // Create thumbnail directory if it doesn't exist
    if (!file_exists(rtrim($path, '/') . '/thumb/')) {
        mkdir(rtrim($path, '/') . '/thumb/', 0777, true);
    }

    // Set source and target paths
    $source_path = rtrim($path, '/') . '/' . $filename;
    $target_path = rtrim($path, '/') . '/thumb/' . $filename;
    ...
    // Initialize and execute image manipulation
