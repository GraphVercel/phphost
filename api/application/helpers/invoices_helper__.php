...
function get_invoice_shortlink($invoice)
{
 ...
 function item_images_load($item_id = null){
    
    $CI = &get_instance();
    if ($item_id != null) {
        $CI->db->select('*')->where('item_id', $item_id);
        $item_images = $CI->db->get(db_prefix() . 'item_images')->result();
 ...
 function item_images_load_against_random_number($key_code = 0){
    
    $CI = &get_instance();
    if ($key_code != 0) {
        $CI->db->select('*')->where('key_code', $key_code);
        $item_images = $CI->db->get(db_prefix() . 'item_images')->result();
        $images = [];
  ...
  function item_img_id_against_random_number($key_code = 0){
 ...
 function get_item_random_number_against_id($id = 0){
    
    $CI = &get_instance();
    if ($id != 0) {
        $CI->db->select('*')->where('id', $id);   
    }
    $query = $CI->db->get(db_prefix() . 'item_images');
    if ($query) {
        return $query->result()[0]->key_code; 
    } else {
        return false; 
    }
